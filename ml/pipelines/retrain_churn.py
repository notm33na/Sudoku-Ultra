"""
Airflow DAG: retrain_churn

Weekly retraining of the churn-predictor (Logistic Regression).
Triggers ml-service /retrain, waits for the MLflow run to finish,
evaluates AUC-ROC and F1 thresholds, then registers and promotes.

Schedule : 0 5 * * 1   (Monday 05:00 UTC — after classifier and regression)
SLA       : test_auc_roc ≥ 0.80, test_f1 ≥ 0.72
"""
...
def evaluate_metrics(**context) -> None:
    ...
    auc = m.get("test_auc_roc", 0.0)
    f1  = m.get("test_f1",      0.0)
    print(f"[evaluate_metrics] auc_roc={auc:.4f}  f1={f1:.4f}")
    if auc < AUC_MIN:
        raise ValueError(f"test_auc_roc {auc:.4f} < threshold {AUC_MIN}")
    if f1 < F1_MIN:
        raise ValueError(f"test_f1 {f1:.4f} < threshold {F1_MIN}")
...
with DAG(
    ...
    description="Weekly churn-predictor retraining with MLflow promotion",
    schedule_interval="0 5 * * 1",
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=["retrain", "ml", "churn"],
) as dag:
