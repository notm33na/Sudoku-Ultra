"""
Airflow DAG: retrain_regression

Weekly retraining of the adaptive-regression model (Gradient Boosting).
Triggers ml-service /retrain, waits for the MLflow run to finish,
evaluates RMSE threshold, then registers and promotes to Production.

Schedule : 0 4 * * 1   (Monday 04:00 UTC — 1 hour after classifier)
SLA       : test_rmse ≤ 0.25, test_r2 ≥ 0.80
"""
...
MODEL_NAME    = "adaptive-regression"
MLFLOW_NAME   = "adaptive-regression"
RMSE_MAX      = 0.25
R2_MIN        = 0.80
N_SAMPLES     = 15000
N_TRIALS      = 50
...
def evaluate_metrics(**context) -> None:
    ...
    rmse = m.get("test_rmse", 999.0)
    r2   = m.get("test_r2",   0.0)
    print(f"[evaluate_metrics] rmse={rmse:.4f}  r2={r2:.4f}")
    if rmse > RMSE_MAX:
        raise ValueError(f"test_rmse {rmse:.4f} > threshold {RMSE_MAX}")
    if r2 < R2_MIN:
        raise ValueError(f"test_r2 {r2:.4f} < threshold {R2_MIN}")
...
with DAG(
    dag_id="retrain_regression",
    default_args=DEFAULT_ARGS,
    description="Weekly adaptive-regression retraining with MLflow promotion",
    schedule_interval="0 4 * * 1",
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=["retrain", "ml", "regression"],
) as dag:
